...
import static io.bpmnsimulator.core.model.Field.ASSIGNEE;

@Slf4j
@Component
class AssigneeValidator implements UserTaskValidator<String> {
    ...
    public List<ProcessSimulationError> validate(@Nonnull final UserTaskValidatorContext<String> context) {
        ...
        final boolean isAssigneeValid = Objects.equals(task.getAssignee(), condition.getExpectedValue());
        if (!isAssigneeValid) {
            final ProcessSimulationError simulationError = ProcessSimulationError.builder()
                    .stepId(context.getStepId())
                    .field(ASSIGNEE)
                    .expectedFieldValue(condition.getExpectedValue())
